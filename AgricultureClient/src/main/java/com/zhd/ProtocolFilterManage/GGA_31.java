...
public class GGA_31 {
	...
	public byte eorw;// 东西经
	public double Longitude; // 经度
	public byte sorn;// 南北纬
	public float speed;// 速度
	public float direction;// 方向
	public float hight;// 高程
	public byte CaculateSatNum;//卫星数
	public byte GPS_state;//解状态
	public byte [] time=new byte[6];//日期
	public byte Mechanicalstate;//机械状态
	public float ousideVe;// 电压
	public byte[] ToByte() {
		...
		ex.AddByte((byte) 0x02);
		ex.AddBytes(Token, false);
		ex.AddByte((byte) 0x00);
		ex.AddByte((byte) 0x2B);
		ex.AddDoubleAsBytes(Latitude);
		ex.AddByte(eorw);
		ex.AddDoubleAsBytes(Longitude);
		ex.AddByte(sorn);
		ex.AddFloatAsBytes(speed);
		ex.AddFloatAsBytes(direction);
		ex.AddFloatAsBytes(hight);
		ex.AddByte(CaculateSatNum);
		ex.AddByte(GPS_state);
		ex.AddBytes(time, false);
		ex.AddByte(Mechanicalstate);
		ex.AddFloatAsBytes(ousideVe);
